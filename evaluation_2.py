...
def evaluate_incremental(X, label, nu, gamma, train_size=20, zero_init=True):
    nu_start = 0.95
    ...
    for train_index, test_index in kfold:
        ...
        cm1_normalized = confusion1.astype('float') / confusion1.sum(axis=1)[:, np.newaxis]
        confusion = output_cf(expected, predicted)
        ...
        if X_train_batch.shape[0] < 15000:
            ...
            cm2_normalized = confusion2.astype('float') / confusion2.sum(axis=1)[:, np.newaxis]
            confusion = output_cf(expected, predicted)
            predicted_prob = clf.decision_function(X)*novelty
        ...
        cm3_normalized = confusion3.astype('float') / confusion3.sum(axis=1)[:, np.newaxis]
        confusion = output_cf(expected, predicted)
        predicted_prob = clf.decision_function(X)*novelty
...
def profile_incremental(X_train, labels=None):
    nu_start = 0.95
    nu = 0.3
    clf = ocsvm.OCSVM("rbf", nu=nu_start, gamma=0.001)
    if nu < nu_start:
        train_size = ceil(len(X_train) * nu / nu_start)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 3:
        ...
        zero_init = True if sys.argv[3].strip() == "zero" else False
    else:
        nu = 0.9
        gamma = 30
        zero_init = True
    ...
    zero_init = True
    X, label = load_ecoli1()
    evaluate_incremental(X, label, nu, gamma, zero_init=zero_init)
